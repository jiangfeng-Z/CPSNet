...
class RevResNet(nn.Module):
    ...
    def sample(self, transfer_module, x_c, x_s, device):
        ...
        assert x_c.size(0) == x_s.size(0)

        with torch.no_grad():
            for i in range(x_c.size(0)):
                content = x_c[i].unsqueeze(0).to(device)  # [1, C, H, W]
                style = x_s[i].unsqueeze(0).to(device)  # [1, C, H, W]

                z_c = self(content)
                z_s = self(style)

                z_cs = transfer_module(z_c, z_s)

                stylized = self(z_cs, forward=False)

                # 循环一致性检查
                z_cs_rev = self(stylized)
                z_csc = transfer_module(z_cs_rev, z_s)
        ...
        self.train()
        return x_c.cpu(), x_s.cpu(), x_cs, x_c_cyc
